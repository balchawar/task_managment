...
Class Action {
	...
	function save_item(){
		...
		// echo $data;exit;
		if(empty($id)){
			$save = $this->db->query("INSERT INTO item_list set $data");
		}else{
			$save = $this->db->query("UPDATE item_list set $data where id = $id");
		}
		if($save){
			return 1;
		}
	...
	function delete_item(){
		extract($_POST);
		$delete = $this->db->query("DELETE FROM item_list where id = $id");
		if($delete){
			return 1;
		}
	}
